...
import { formatNumbers, formatBytes } from '../viewmodels/utils/format.mjs';
import copyAssets from '../app/utils/copyassets.mjs';
import inline from '../app/utils/templateHelpers/inline.mjs';
import pluralize from '../app/utils/templateHelpers/pluralize.mjs';
import ruleSizeStat from '../viewmodels/statproject/ruleSizeStat.mjs';
import selectorsListTops from '../viewmodels/statproject/selectorsListTops.mjs';
import totals from '../viewmodels/statproject/totals.mjs';
import uniques from '../viewmodels/statproject/uniques.mjs';
import parseSpaces from '../viewmodels/statproject/parseSpaces.mjs';
import sortSizes from '../viewmodels/statproject/sortSizes.mjs';
import prepareFontData from '../models/componentstat/utils/getFont.mjs';
import guessSelectorType from '../models/componentstat/utils/guessSelectorType.mjs';
import prepareDisplayName from '../viewmodels/statcomponent/prepareDisplayName.mjs';
import getConstantsUsage from '../viewmodels/statcomponent/getConstantsUsage.mjs';
        ...
        describe('format()', function() {
            ...
            describe('numbers', function() {
                it('should return proper formatted numbers with 0', function() {
                    assert.strictEqual(formatNumbers(0), 0);
                });
                it('should return proper formatted numbers more than 4', function() {
                    assert.strictEqual(formatNumbers(500), '500');
                });
                it('should return proper formatted numbers less than 4', function() {
                    assert.strictEqual(formatNumbers(3), '3');
                });
                it('should return proper formatted numbers with float point', function() {
                    assert.strictEqual(formatNumbers(3.000000002), '3.0');
                });
                it('should return proper formatted numbers with float point', function() {
                    assert.strictEqual(formatNumbers(0.6000000000000001), '600m');
                });
                it('should return proper formatted numbers with thousand numbers', function() {
                    assert.strictEqual(formatNumbers(20000), '20k');
                });
                it('should return proper formatted numbers million numbers', function() {
                    assert.strictEqual(formatNumbers(1000000), '1.0M');
                });
            });
            describe('bytes', function() {
                it('should return proper formatted bytes', function() {
                    assert.strictEqual(formatBytes(600), '600B');
                });
                it('should return proper formatted kilo bytes', function() {
                    assert.strictEqual(formatBytes(1000), '1000B');
                });
                it('should return proper formatted kilo bytes', function() {
                    assert.strictEqual(formatBytes(1128), '1kB');
                });
                it('should return proper formatted mega bytes', function() {
                    assert.strictEqual(formatBytes(1000000), '977kB');
                });
                it('should return proper formatted mega bytes', function() {
                    assert.strictEqual(formatBytes(1128000), '1MB');
                });
                it('should return proper formatted giga bytes', function() {
                    assert.strictEqual(formatBytes(1128000000), '1GB');
                });
            });
        });
...
`;
    ...
    describe('componentstat', function() {
        describe('getFont', function() {
            ...
            tests.forEach(function(test) {
                it('should return right value in `' + test.name + '` case', function() {
                    ...
                    assert.deepStrictEqual(expectedResult, prepareFontData(test.property));
                });
            });
        });
        describe('guessType', function() {
            ...
            describe('single selector', function() {
                ...
                const getResult = selector => guessSelectorType(selector, componentPrefixRegExp);
    ...
    describe('statcomponent', function() {
        describe('prepareDisplayName', function() {
            // const prepareDisplayName = require(cwd + '/viewmodels/statcomponent/prepareDisplayName');

            it('should return proper display names for "fontFamily"', function() {
                assert.strictEqual(prepareDisplayName('fontFamily', true), 'Font family');
                assert.strictEqual(prepareDisplayName('fontFamily', false), 'Font families');
            });
            it('should return proper display names for "fontSize"', function() {
                assert.strictEqual(prepareDisplayName('fontSize', true), 'Font size');
                assert.strictEqual(prepareDisplayName('fontSize', false), 'Font sizes');
            });
            it('should return proper display names for "backgroundColor"', function() {
                assert.strictEqual(prepareDisplayName('backgroundColor', true), 'Background');
                assert.strictEqual(prepareDisplayName('backgroundColor', false), 'Backgrounds');
            });
            it('should return proper display names for "mediaQuery"', function() {
                assert.strictEqual(prepareDisplayName('mediaQuery', true), 'Media query');
                assert.strictEqual(prepareDisplayName('mediaQuery', false), 'Media queries');
            });
            it('should return proper display names for "boxShadow"', function() {
                assert.strictEqual(prepareDisplayName('boxShadow', true), 'Box shadow');
                assert.strictEqual(prepareDisplayName('boxShadow', false), 'Box shadow');
            });
            it('should return proper display names for any other values', function() {
                assert.strictEqual(prepareDisplayName('margin', true), 'margin');
                assert.strictEqual(prepareDisplayName('margin', false), 'margins');
            });
        });
        describe('getComponentStat', function() {
            // const getConstantsUsage = require(cwd + '/viewmodels/statcomponent/getConstantsUsage');
            ...
            it('should return counter for suggest right props', function() {
                ...
                assert.deepStrictEqual(getConstantsUsage('padding', valueList, constants), expectedResult);
            });
            ...
            it('should return undefined in case of prop not much constants map', function() {
                const expectedResult = undefined;
                assert.deepStrictEqual(getConstantsUsage('overflow', valueList, constants), expectedResult);
            });
        });
    });
});
