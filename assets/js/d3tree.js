...
(function () {
    ...
    // const rawdata = [
    //     {'file': 'fatface', 'parent': '', 'size': '0'},
    //     {'file': 'style.css', 'parent': 'fatface', 'size': '10'}
    // ];
    //
    ...
    const data = d3.stratify()
        .parentId(d => d.id.substring(0, d.id.lastIndexOf('.')))(rawdata)
        .sort((a, b) => (a.height - b.height) || a.id.localeCompare(b.id));

    d3.tree()
        .separation((a, b) => (a.parent === b.parent ? 1 : 2) / a.depth)
        // .size([width / 2, height / 2])(data);
        .size([2 * Math.PI, 500])(data);

    data.descendants().forEach(d => d.y = d.depth * 300); // normalize levels
