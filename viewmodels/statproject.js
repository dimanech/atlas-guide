...
function statProject(stat) {
    ...
    function sortSizes(size) {
        ...
        const convertToAbsoluteUnits = function(value) {
        ...
        size.forEach(item => {
            const abs = convertToAbsoluteUnits(item);
            sorted.push({
                orig: item,
                abs: abs,
                // TODO: refactor me
    ...
    function uniquesChart(stats) {
        ...
        // Font size
        const allFontSizes = stats.declarations.getAllFontSizes();
        uniques.fontSize = {
            total: allFontSizes.length,
            unique: _uniq(allFontSizes).length
        };
        if (uniques.fontSize.total > uniques.max) {
            uniques.max = uniques.fontSize.total;
        }
        uniques.fontSize.chart = renderUniquesChart(uniques.fontSize, uniques.max);

        // Font families
        // TODO: refactor me
        const allFontFamilies = stats.declarations.getAllFontFamilies();
        uniques.fontFamily = {
            total: allFontFamilies.length,
            unique: _uniq(allFontFamilies).length
        };
        if (uniques.fontFamily.total > uniques.max) {
            uniques.max = uniques.fontFamily.total;
        }
        uniques.fontFamily.chart = renderUniquesChart(uniques.fontFamily, uniques.max);
    ...
    function specificityChart(stats) {
        ...
        return {
            ...
            //'chart': renderLineChart(stats.selectors.getSpecificityGraph()),
